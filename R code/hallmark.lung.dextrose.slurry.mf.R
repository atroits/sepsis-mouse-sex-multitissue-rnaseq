...
dextrose <- read_csv("sasha/lung.dextrose.male.female.csv")

slurry <- read_csv("sasha/lung.slurry.male.female.csv")
...
# Set levels of the combined factor based on average NES. UNCOMMENT BELOW FOR ARRANGE WITHIN EACH CATEGORY BY NES
...
# Update the Pathway factor based on ordered Pathway_combined
pathway_levels <- combined %>%
  distinct(Pathway_combined, Pathway) %>%
  arrange(factor(Pathway_combined, levels = pathway_order)) %>%
  pull(Pathway)

combined$Pathway <- factor(combined$Pathway, levels = pathway_levels)
...
p <- ggplot(combined, aes(x = NES, y = Pathway)) +
  ...
  #  facet_grid(. ~ Organ) +
  labs(
    title = "GSEA Hallmark Pathways: Effect of Biological Sex on Septic Response in the Lung",
...
ggsave("GSEA_dotplot_lung_dextrose_slurry_mf.png", plot = p, width = 20, height = 24, dpi = 300)
