...
# 2. Load counts file
# -------------------------
counts <- read_tsv("~/Desktop/RNASEQ/DESeq2 data YMF/kidney.full.ymf/counts.txt")
counts <- counts %>%
...
# 4. Load DE results
# -------------------------
de_female <- read_csv("~/Desktop/RNASEQ/DESeq2 data YMF/biol sex/gsa_result.male.female.kidney.csv") %>%
  rename(
...
de_male <- read_csv("~/Desktop/RNASEQ/DESeq2 data YMF/biol sex/gsa_result.male.female.kidney.csv") %>%
  rename(
...
rownames(sample_groups) <- c("S42","S46","S81","S82",
                             "S47","S48","S75","S77","S79",
                             "S43","S44","S80",
                             "S74","S76","S78")
